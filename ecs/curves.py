...
def CurveList():
    '''Returns the names of all the stored curves.'''
    return ['NIST P-192', 'NIST P-224', 'NIST P-256', 'NIST P-384', 'NIST P-521']

def SelectCurve(pick):
    '''Returns the specified Elliptic Curve, a point on the curve, and the order of the point.'''
    if pick == 'NIST P-192':
        return P192()

    elif pick == 'NIST P-224':
        return P224()

    elif pick == 'NIST P-256':
        return P256()

    elif pick == 'NIST P-384':
        return P384()

    elif pick == 'NIST P-521':
        return P521()
    
    else:
        print 'Error: No curve named ' + pick + ' found.'
