...
def ExtractImageInfo(im, im_type):
    ...
    if im_type == 'public key':
        ...
        curve_name = ''.join([chr(num) for num in info_list[4:4+curve_name_length]])
        ...
        t = ''.join([chr(num) for num in info_list[5:33]])
        curve_name = ''.join([chr(num) for num in info_list[33:33+curve_name_length]])
        enc = ''.join([chr(num) for num in info_list[33+curve_name_length:33+curve_name_length+enc_length]])
